...
\chapter{Multi-core Programming} \label{multicorechapter} %Chapter outline is work in progess..
...
Some text will come here! \\
\section{Scheduling}  \label{schedulingsection}%and schedulers
Some text will come here! \\
...
\subsection{Implemented Online Timing Features} \label{onlinetiming}
...
One of the most important optimization goals include reducing the energy consumption. To reduce the energy consumption one should have a properly utilized software which is achieved through balancing the CPU load through all the cores of the system. In order to understand this, Power Consumption Application Manual for XS1-L devices \cite{xs1energymanual} should be studied well. Since CPU hardware related power consumption is out of reach for this research, we focus on resource related power consumption. A good illustration of how power consumption changes is shown in the Figure \ref{fig:xs1energy} \cite{xs1energymanual}.
...
\begin{itemize}
	\item ${ST\textsubscript{avg}}$ in seconds is calculated simply by using the following equation, given that there are ${N\textsubscript{process}}$ processes measured:
	\begin{equation}
