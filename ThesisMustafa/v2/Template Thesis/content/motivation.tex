...
Linux uses a Round-robin based scheduler called Completely Fair Scheduler (CFS). The goal of CFS is to maximize overall CPU utilization while also maximizing interactive performance and being fair to all the processes and threads \cite{cfs}. In order to ensure fairness, a tree structure of process execution times are created and sorted. Every time, the process which is executed the least time is picked from the tree and executed until preemption \cite{cfs}.
