...
\newpage\lstinputlisting[caption=Proximity sensing task, label=lst_proximitysensing, style=xc]{content/listings/lst_proximitysensing.txt}
...
\section{System Limitations} %hw-sw limitations, xmos hw & xta tracing limitations, overall tracing limitations, perf limitations, granularity limitations due to process-based distribution, image processing has significantly more load,
...
\begin{itemize}
	...
	\item Sporadic Activations: %non-determinism & randomness
	\item Limitations of perf profiler:
	\item Limitations of process-based distribution: %reverse engineering, high granularity, image processing app
	\item Limitations of XTA:
	\item Hardware Limitations: %Rpi clock can take only 600mhz and 1.2GHz
\end{itemize}
...
\end{itemize}
After the modeling, partitioning (pre-partitioning is performed automatically before partitioning), task generation, and mapping is performed by using APP4MC to obtain the distribution results.
...
\begin{table}[h!]
	\begin{tabular}{|l|l|l|l|l|}
		...
		\hline
	\end{tabular}
	\caption{Distribution-1 details in High-level Module}
	\label{tbl_distr1_hlm}
\end{table}
...
\begin{table}[h!]
	\begin{tabular}{|l|l|l|l|l|l|}
		...
		\hline
	\end{tabular}
	\caption{Distribution-1 details in Low-level Module}
	\label{tbl_distr1_llm}
\end{table}
