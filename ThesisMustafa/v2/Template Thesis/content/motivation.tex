...
\begin{itemize}
	...
	\begin{itemize}
		\item The real-time capability of Raspberry Pi is very low. Implemented schedulability and tracing features as well as Linux kernel introduces huge overheads. This effects the  obtained results. Using a real-time kernel with minimal overhead would produce better results.
		\item Main purpose with APP4MC is that it is used as a supplementary tool for standards such as AUTOSAR. Using the tool with a non-industry board and distribution (Raspberry Pi, Raspbian) is effecting the results because scheduling is left entirely to Linux kernel and it is not necessarily real-time.
...
\subsubsection{LLM-Distribution-APP4MC}
Some text
\llmdistrA

\subsubsection{LLM-Distribution-Hand}
Some text
\llmdistrA
