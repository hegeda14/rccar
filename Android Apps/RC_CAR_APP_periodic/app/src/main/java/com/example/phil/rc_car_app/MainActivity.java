...
public class MainActivity extends Activity {
    ...
    private SeekBarVertical speedBar;
    public static TextView speedValueField, angleValueField, connectionView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Thread t = new Thread(new ControlRCCar());
        t.start();
    }
    ...
    private void seekBarOne() {
        speedBar = (SeekBarVertical) findViewById(R.id.seekBar1);
        speedValueField = (TextView) findViewById(R.id.textFieldSeekBox1);
        speedBar.setMax(speedValueMax);
        speedBar.setProgress(speedValueStart);
        speedValueField.setText(Integer.toString(speedValueStart));
        speedBar.setOnSeekBarChangeListener(new SeekBarVertical.OnSeekBarChangeListener() {
    ...
    private void seekBarTwo() {
        angleBar = (SeekBar) findViewById(R.id.seekBar2);
        angleValueField = (TextView) findViewById(R.id.textFieldSeekBox2);
        angleBar.setMax(angleValueMax);
        angleBar.setProgress(angleValueStart);
    ...
    private String sendRCCar(){
        if(direction != ' ') {
            ...
            return command;
             ...
             connectionView.setText("please set an direction");
        }
        return " ";
    }
    ...
    /**
     * new class for the endless loop to send a periodic command to control the rc car
     */
    private class ControlRCCar implements Runnable{
        @Override
        public void run() {
            while (!Thread.currentThread().isInterrupted()){
                //need a new runnable to be able to display the error messages to the connection view field
                connectionView.post(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            //create the command line for the RC Car
                            sendRCCar();
