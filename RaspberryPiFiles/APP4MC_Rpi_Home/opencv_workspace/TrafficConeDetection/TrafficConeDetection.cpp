...
int main(int argc, char *argv[])
{
    ...
    if(connect_flag == 1)
    {
	    ...
	    server = gethostbyname("192.168.20.60");
	    if (server == NULL) {
...
Mat imgResultingThresh;
  ...
  int third_maximum_idx;
	...
	printf("Starting...\n");
	  ...
	  //TRACKING EKLENECEK!!!!
	  if(abs(detections_y[maximum_idx] - detections_y[second_maximum_idx])<10 && second_maximum>0 && maximum > 0)
	  {
		opening_length = abs(detections_x[maximum_idx]-detections_x[second_maximum_idx]);
		if(detections_x[maximum_idx]>detections_x[second_maximum_idx])
	  		opening_midpoint = (max(detections_x[maximum_idx],detections_x[second_maximum_idx])+detections_w[maximum_idx])/2;
		else
			opening_midpoint = (max(detections_x[maximum_idx],detections_x[second_maximum_idx])+detections_w[second_maximum_idx])/2;
	  }
	  else if(abs(detections_y[third_maximum_idx] - detections_y[second_maximum_idx])<10 && second_maximum>0 && third_maximum > 0)
	  {
		opening_length = abs(detections_x[third_maximum_idx]-detections_x[second_maximum_idx]);
	  	if(detections_x[third_maximum_idx]>detections_x[second_maximum_idx])
	  		opening_midpoint = (max(detections_x[third_maximum_idx],detections_x[second_maximum_idx])+detections_w[third_maximum_idx])/2;
		else
			opening_midpoint = (max(detections_x[third_maximum_idx],detections_x[second_maximum_idx])+detections_w[second_maximum_idx])/2;
	  }
	  else
	  {
		opening_midpoint = -1;
	  ...
	  printf("mid = %d\n",opening_midpoint);
	if(opening_midpoint != -1)
	{
		if(opening_midpoint > PICTURE_MIDPOINT - 20)
		{
			//Orta aciklik
			if (opening_midpoint > PICTURE_MIDPOINT + 20)
			{
